...
from helper.menu_helper import assert_schema_validation
...
def test_address_elegibility_should_match_schema(address_eligibility_response):
    ...
    """
    logger.info("Testing address eligibility schema validation")

    cep = "01310930"
    number = "100"

    response = address_eligibility_response(cep, number)

    assert_schema_validation(response, ADDRESS_ELIGIBILITY_SUCCESS_SCHEMA)
...
def test_address_elegibility_should_return_status_eligible_for_especific_zipcode(address_eligibility_response):
    ...
    """
    logger.info("Testing address eligibility status validation for specific zipcode")
...
def test_address_elegibility_should_return_status_eligible_for_generic_zipcode(address_eligibility_response):
    ...
    """
    logger.info("Testing address eligibility status validation for generic zipcode")
...
def test_address_elegibility_should_return_status_not_eligible_for_non_covered_zipcode(address_eligibility_response):
    ...
    """
    logger.info("Testing address eligibility status validation for non-covered area zipcode")
